...
export default Pedidos;import React, { useState, useEffect, useRef } from 'react';
import { db } from '../firebase';
...
function Coordinador({ usuario, onVolver }) {
  ...
  async function aceptarDespacho(pedidoId) {
    ...
    try {
      ...
      // Si es sin transportista: puede quedar Programado o prog-parcial
      // Si es con transportista: queda prog-parcial hasta asignar
      const hayPendiente = nuevosDespachos.some(d => d.estado === 'Aceptado-pendiente');
      const nuevoEstado = hayPendiente
        ? 'prog-parcial'
        : volDespachado >= Number(p.volumen) ? 'Programado' : 'prog-parcial';
      ...
      if (esSinTransportista) {
        alert('✓ Despacho aceptado y escrito en el plan.');
      } else {
                ...
                {(p.despachos || []).map((d, i) => {
                  ...
                  return (
                    <div key={i} style={{ ...styles.despachoItem, borderColor: d.estado === 'En espera' ? '#EF9F27' : d.estado === 'Aceptado-pendiente' ? '#F59E0B' : '#E5E7EB' }}>
                      ...
                      {/* Panel asignar transportista — solo si NO es retiro del cliente / proveedor */}
                ...
                {saldo(p) > 0 && p.estado !== 'Suspendido' && (
                  <div style={styles.nuevoDespacho}>
                    <div style={styles.despachosTitle}>
                      {sinTransportista(p.tipo) ? '✓ Aceptar pedido — escribir en plan' : '✓ Aceptar pedido — escribir en plan'}
                    </div>
